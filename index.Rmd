...
While the land use maps clipped for each watershed are interesting to look at, let's access the data behind them.

```{r, warning=F, message=F,fig.cap="Extracting Land Use Raster Data from within Watershed Boundaries"}
#Conesus
...
```{r, warning=F, message=F,fig.cap="Summarizing Land Use Data for All 11 Watersheds"}
...
datatable(table1, options = list(pageLength = 5))
```
Above, the data was moved around and manipulated in order to create a table summarizing the analysis results. For each lake, the data table shows the proportion of watershed made up by each land use type, the total watershed area in square kilometers, the total surface area of the lake in square kilometers, and the total number of weeks that each lake had recorded cyanobacterial blooms from 2012-2017. With these data summarized into a table, it is possible to dig into it in order to look for statistical patterns. In this way, it might be possible to understand why the eleven lakes that are so close in proximity are affected so differently by cyanobacteria.
# Conclusions
