...
Next, we'll need to clip our land use raster image 11 times using the 11 separate waterhed polygons. This will enable us to visualize the land use separately for each waterhed and analyze the data of each watershed separately.
...
table1=as.data.frame(rbind(conesus_matrix,otisco_matrix,skaneateles_matrix,honeoye_matrix,canandaigua_matrix,owasco_matrix,cayuga_matrix,seneca_matrix,keuka_matrix,canadice_matrix,hemlock_matrix))
row.names(table1)=c(lakes)
colnames(table1)=c('Urban','Agriculture','Water','Forest','Bare')
table1$Total=c(rowSums(table1))
table1$U_prop=c(table1$Urban/table1$Total)
table1$U_prop=round(table1$U_prop,2)
table1$A_prop=c(table1$Agriculture/table1$Total)
table1$A_prop=round(table1$A_prop,2)
table1$W_prop=c(table1$Water/table1$Total)
table1$W_prop=round(table1$W_prop,2)
table1$F_prop=c(table1$Forest/table1$Total)
table1$F_prop=round(table1$F_prop,2)
table1$B_prop=c(table1$Bare/table1$Total)
table1$B_prop=round(table1$B_prop,2)
table1$Urban=NULL
table1$Agriculture=NULL
table1$Water=NULL
table1$Forest=NULL
table1$Bare=NULL
table1$Total=NULL
...
canadice@data$Area
hemlock@data$Area
table1$Wtshd_Area=c(181,177,206,178,489,542,2034,1432,367,45,112)
#Lake Area measurements pulled from Google Search
table1$Lake_Area=c(13.84,8.29,35.61,7.17,43.50,26.97,172.00,173.30,47.47,2.63,7.28)
table1$LW_prop=table1$Lake_Area/table1$Wtshd_Area
table1$LW_prop=round(table1$LW_prop,2)
#Bloom weeks are the summed bloom weeks from each lake between 2012-2017
table1$BlmWks=c(22,2,5,68,11,48,20,13,6,0,0)
```
...
plot(x = table1$U_prop,y = table1$BlmWks,main='Urban Watershed Proportion vs. Bloom Weeks')
abline(lm(table1$BlmWks~table1$U_prop), col="red")
# Proportion of Watershed that is Agricultural vs. # weeks of recorded bloom
plot(x = table1$A_prop,y = table1$BlmWks,main='Agricultural Watershed Proportion vs. Bloom Weeks')
abline(lm(table1$BlmWks~table1$A_prop), col="red")
# Proportion of Watershed that is Water vs. # weeks of recorded bloom
plot(x = table1$W_prop,y = table1$BlmWks,main='Water Watershed Proportion vs. Bloom Weeks')
abline(lm(table1$BlmWks~table1$W_prop), col="red")
# Proportion of Watershed that is Forest vs. # weeks of recorded bloom
plot(x = table1$F_prop,y = table1$BlmWks,main='Forest Watershed Proportion vs. Bloom Weeks')
abline(lm(table1$BlmWks~table1$F_prop), col="red")
# Proportion of Watershed that is Bare Land vs. # weeks of recorded bloom
plot(x = table1$B_prop,y = table1$BlmWks,main='Bare Land Watershed Proportion vs. Bloom Weeks')
abline(lm(table1$BlmWks~table1$B_prop), col="red")
# Size of Watershed vs. # weeks of recorded bloom
plot(x = table1$Wtshd_Area,y = table1$BlmWks,main='Watershed Size (km2) vs. Bloom Weeks')
abline(lm(table1$BlmWks~table1$Wtshd_Area), col="red")
# Size of Lake vs. # weeks of recorded bloom
plot(x = table1$Lake_Area,y = table1$BlmWks,main='Lake Surface Area (km2) vs. Bloom Weeks')
abline(lm(table1$BlmWks~table1$Lake_Area), col="red")
# Ratio of Lake surface area to Watershed Area vs. # weeks of recorded bloom
plot(x = table1$LW_prop,y = table1$BlmWks,main='Lake:Watershed Proportion vs. Bloom Weeks')
abline(lm(table1$BlmWks~table1$LW_prop), col="red")

```
Based on the scatter plots and best fit linear regression lines, it appears that the hypotheses that were made do not hold much merit. It was hypothesized that lakes demonstrating more frequent cyanobacterial blooms would have higher agricultural and urban watershed proportions since agricultral and urban lands tend to encourage surface runoff of nutrient-rich waters. The scatterplots and linear regression lines, however, show little relationship between both urban percentage and agricultural percentage with total number of recorded bloom weeks.
...
In addition, the scatter plots and linear regression lines suggest that as the proportion of watershed that is made up of water increases, bloom frequency tends to decrease. Similarly, the relationships between Lake:Watershed proprtion and frequency of blooms demonstrates the same trend-- small lakes with large watersheds tend to have higher bloom frequencies than large lakes in small watersheds.
